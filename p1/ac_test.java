...
public class ac_test {
	...
	public static class TrieDLB<Value>
	{
	...
	{
		if(key == null)
			throw new IllegalArgumentException("The String argument to put() is null");
		if(val == null)
		{
	...
	private void collect(Node x, String prefix, LinkedQueue<String> queue)
	{
		...
		{			queue.enqueue(prefix);
					//System.out.println("enqueue: " + prefix);
		}
...
public static void main(String[] args) {
		...
		try{
		readFile("dictionary.txt", st);
		}catch (FileNotFoundException e) { 
		System.err.println("File dictionary.txt not found");
		}
		try{
			readFile("user_history.txt", userIn);
		}
		catch(FileNotFoundException e)
		{
			System.err.println("File user_history not found");
		}
				...
				do{
					...
					if(again == false)
					{
						...
						if(input.charAt(0) == '$')
						{
							...
							System.out.println();
							save(prefix);
							userIn.put(prefix, userIn.size()+1);
		...
		int i = 0;
			...
			if(i < 5)
			{
				...
				while(!(temp.isEmpty()) && i < 5)
				{
					...
					for(int a = 0; a < i; a++)
					{
						if(suggestions[a] != null && suggestions[a].equals(word))
						{
							if(!temp.isEmpty())
							{
								word = temp.dequeue();
